...
Teaches the Host-side `staqex.toml` resource manifest and simulator budget check
...
```bash
python3 -m compiler.staqex run examples/basics/B14_resource_profile/main_resource_profile.sqx --seed 0
```
...
The local `staqex.toml` sets simulator policy to `Warn` so an exceeded estimate
