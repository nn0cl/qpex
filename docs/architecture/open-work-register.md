...
| QASM function-call lowering | Phase 3 Refactor complete | [LISS-0049](../issues/LISS-0049-qasm-function-call-lowering.md) | Split from LISS-0021. Architecture Path selected Option B (2026-07-25): calling a user-defined `fn` from `main` now rejects with `QASM_FUNCTION_CALL_UNSUPPORTED` (backend `reject_code` and CLI exit code) instead of silently falling back to the empty-program sketch. Option A (correct inlined output) remains a possible future follow-up. Adjudicator final review pending. |
