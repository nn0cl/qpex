...
| Typed second quantization | Phase 3 Refactor complete | [LISS-0032](../issues/LISS-0032-typed-second-quantized-operators.md); [ADR 0093](adr/0093-jordan-wigner-numerical-mapping.md) | Fermion/boson/spin/qubit family boundaries, statistics provenance, and explicit mapping metadata are shipped. Jordan-Wigner numerical mapping for `FermionOperator` (one-body and two-body terms) is shipped: a mapped Hamiltonian runs on the SV simulator and emits QASM. Bravyi-Kitaev, Boson, and Spin mappings, and exchange-law normalization beyond canonical ordering, remain deferred. Adjudicator final review pending. |
