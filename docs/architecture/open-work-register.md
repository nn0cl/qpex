...
| Finite mathematical binder lowering | Phase 3 reviewed; **superseded in scope** | [LISS-0043](../issues/LISS-0043-finite-binder-lowering.md); [ADR 0088](adr/0088-finite-binder-lowering.md) | Inclusive Open ranges lower the restricted Pauli nearest-neighbor sum with diagnostics, resource guard, and provenance. **ADR 0088 Decision 3's promise of "a concrete Pauli Operator tree suitable for the existing Hamiltonian/Suzuki path" is not met by the implementation** (an inspection `dict` is produced instead) — tracked as a bug in LISS-0052. The accepted body scope is also too narrow to write canonical models; superseded by [ADR 0096](adr/0096-indexed-operator-and-binder-surface.md) and [WP-0024](../work-plans/WP-0024-indexed-operator-and-binder-surface.md). |
