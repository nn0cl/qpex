...
| Operator Pauli-atom-call parsing gap | proposed | [LISS-0051](../issues/LISS-0051-operator-pauli-atom-call-parse-gap.md) | `Operator H = Z(0) * Z(1)` (no leading numeric coefficient) mis-parses as a generic `BinOp`/`Call` instead of the Operator-DSL `OpBin`/`OpPauli`, because the parser's factory-call heuristic does not exempt the reserved Pauli atom names (`I`/`X`/`Y`/`Z`/`hop`). Affects both the SV and QASM paths; found during LISS-0011 review. Fix identified (exclude reserved atoms from the heuristic); awaiting Phase 1 Red. |
