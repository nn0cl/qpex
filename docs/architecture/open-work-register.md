...
| Operator Pauli-atom-call parsing gap | Complete | [LISS-0051](../issues/LISS-0051-operator-pauli-atom-call-parse-gap.md) | `Operator H = Z(0) * Z(1)` (no leading numeric coefficient) now correctly parses as `OpBin`/`OpPauli` instead of a generic `BinOp`/`Call`; the parser's factory-call heuristic excludes the reserved Pauli/`hop` atom names. Found during LISS-0011 review; affected both the SV and QASM paths. Adjudicator final review approved 2026-07-25. |
...
| Finite mathematical binder lowering | Phase 3 complete | [LISS-0052](../issues/LISS-0052-binder-lowering-execution-wiring.md); [LISS-0043](../issues/LISS-0043-finite-binder-lowering.md); [ADR 0088](adr/0088-finite-binder-lowering.md) | Inclusive finite sums retain inspection provenance and now also produce executable `OpExpr` trees consumed by the SV and QASM Hamiltonian paths. Literal indexed Pauli sites are supported wherever site-qualified Pauli operators are valid. |
| Binder composition and honest deferral | Phase 3 complete | [LISS-0053](../issues/LISS-0053-binder-composition-and-honest-deferral.md); [ADR 0096](adr/0096-indexed-operator-and-binder-surface.md) | Composed finite sums and named scalar coefficients lower through existing execution paths; deferred binders such as `product` emit hard `BINDER_LOWERING_UNSUPPORTED` diagnostics instead of silently disappearing. |
| Indexed operator and binder surface (final form) | Accepted design; implementation proposed | [ADR 0096](adr/0096-indexed-operator-and-binder-surface.md); [WP-0024](../work-plans/WP-0024-indexed-operator-and-binder-surface.md); LISS-0052…LISS-0057 | One bracket notation `Op[i]` everywhere; binder body is a full operator expression (subsuming `+`, named coefficients, second-quantized atoms, and nested binders, so multi-index sums need no dedicated syntax); binders compose as expressions; explicit `next`/`wrap` boundary accessors; `where` guards; empty domains yield symbolic identity elements materialised late; normative expansion/aggregation order. Unblocks TFIM, Heisenberg/XXZ, Hubbard, molecular electronic structure, and periodic rings. |
| Acting-space typing | proposed — design intake only, **unscheduled** | [LISS-0058](../issues/LISS-0058-acting-space-typing.md); ADR 0096 D12 | Qubit count is currently inferred by scanning for the maximum Pauli site index, so site-free expressions, identity-only expressions, unused high qubits, and multi-register systems are all mis-handled. Needs its own ADR; LISS-0056 ships only the minimal context-determined path empty identities require. |
