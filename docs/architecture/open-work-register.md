...
| Operator Pauli-atom-call parsing gap | Phase 2 Green complete | [LISS-0051](../issues/LISS-0051-operator-pauli-atom-call-parse-gap.md) | `Operator H = Z(0) * Z(1)` (no leading numeric coefficient) now correctly parses as `OpBin`/`OpPauli` instead of a generic `BinOp`/`Call`; the parser's factory-call heuristic excludes the reserved Pauli/`hop` atom names. Found during LISS-0011 review; affected both the SV and QASM paths. Phase 3 Refactor / Adjudicator final review pending. |
