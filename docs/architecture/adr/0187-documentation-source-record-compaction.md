...
2. **Historical Issues, Work Plans, and Traces are index records, not current
   files.** When a record is completed, closed, superseded, or otherwise
   historical, its current-tree file is deleted and the compression map retains
   its identifier, canonical destination, baseline tag, full source commit, and
   original path. The map is the only current-tree pointer for the deleted
   record.
...
4. **Keep unresolved records full.** Issues, Work Plans, or Traces that are
   ...
   unresolved Issue is never deleted merely because it is old.
...
- Unresolved Issues remain actionable and readable in place; historical Issues
   retain their identifiers in the map without duplicating settled narrative.
- Historical source paths remain recoverable through the map without retaining
  duplicate narratives or redirect stubs in the working tree.
