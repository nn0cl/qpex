...
| Topic | Was | Now |
|---|---|---|
| Further `\|>` / currying (fusion, partial-app values) | no-further-ship | **partial shipped** — unary bare ADR 0122; Partial `_` ADR 0123; stepwise Call fill ADR 0131; fusion still out |
| Further trait `impl` / effect rows / specialization | no-further-ship | **design boundary** ADR 0128 (no Red specialization yet) |
| SI scale beyond $(L,M,T)$ tags | permanent-out | **base dims** ADR 0121; **explicit `to`** ADR 0124 + wave-2 ADR 0129 + `eV`↔`J` ADR 0132; °C still out |
