...
| Slice | Scope | Size |
|---|---|---|
| **A** | `HirLinearVerifier` port + `LINEAR_DUPLICATE_USE` diagnostic (Red → Green → Refactor) | M |
| **B** | `LINEAR_IMPLICIT_DISCARD` diagnostic; ancilla lifetime tracking within fun scope | M |
...
## Design decisions required before Phase 1

1. **Quantum-typed binding identification**: which `Ty` values are considered
   "linear" (quantum)? Propose: `Ty.KET`, `Ty.DENSITY`, `Ty.STATE` subtypes —
   Adjudicator confirmation needed.
2. **Scope unit**: linear analysis is per-`fun` body only (no inter-procedural
   in this Issue) — confirm.
3. **Uncomputation witness representation**: `HirDecl.effects` gains
   `"Uncompute"` tag vs. a separate `uncompute_witnesses` field — confirm.
4. **Simulator-equivalence check**: exact zero-amplitude tolerance or
   configurable threshold? Propose: exact (amplitude < 1e-12) — confirm.
...
- [ ] Slice A plan approval → Phase 1 Red
- [ ] Slice B plan approval
