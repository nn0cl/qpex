...
| **C** | CLI `migrate` + stdout / `--write` / `--check` (formatter emit → LISS-0072) | **complete** (Red→Green→Refactor) |
...
- [x] Approve Phase 2 Green (`cmd_migrate` wiring).
- [x] Authorize Phase 3 Refactor (readability only; no behavior change).
